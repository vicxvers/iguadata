...
function App() {
  ...
  useEffect(() => {
    ...
    setDataLoading(true);
    Promise.all([fetchContractsSnapshot(), fetch(jsonAssetUrl('/json/empreses.json')).then(res => {
      if (!res.ok) throw new Error(`Empreses HTTP ${res.status}`);
      return res.json();
    }), fetchEmpresaAliases()]).then(async ([snapshotRows, existingEmpreses, empresaAliases]) => {
      if (cancelled) return;
